...
import { useCorTableController } from "./CorTable.controller";
import { CorDTO, UserDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { useAppSelector } from "@application/store";
import { CorAddDialog } from "../../Dialogs/CorAddDialog";
//import {UserUpdateDialog} from "../../Dialogs/UserUpdateDialog"
/**
 * This hook returns a header for the table with translated columns.
 */
const useHeader = (): { key: keyof CorDTO, name: string }[] => {
    const { formatMessage } = useIntl();

    return [
        //{ key: "date", name: formatMessage({ id: "globals.data" }) },
        { key: "codCor", name: formatMessage({ id: "globals.codCor" }) },
        { key: "meserie", name: formatMessage({ id: "globals.meserie" }) }
    ]
};
 ...
 */
const getRowValues = (entries: CorDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const CorTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay } = useCorTableController(); // Use the controller hook.
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <CorAddDialog /> {/* Add the button to open the COR add modal. */}
