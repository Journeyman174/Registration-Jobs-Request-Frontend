...
import { useStudiiTableController } from "./StudiiTable.controller";
import { StudiiDTO, UserDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { useAppSelector } from "@application/store";
import { StudiiAddDialog } from "../../Dialogs/StudiiAddDialog";
import { StudiiUpdateDialog} from "../../Dialogs/StudiiUpdateDialog"
import { ConfirmProvider } from "material-ui-confirm";
import { ConfirmDialogStudii } from "../../Dialogs/ConfirmDialogStudii";
//import {UserUpdateDialog} from "../../Dialogs/UserUpdateDialog"
/**
 * This hook returns a header for the table with translated columns.
 */
const useHeader = (): { key: keyof StudiiDTO, name: string }[] => {
    const { formatMessage } = useIntl();

    return [
        { key: "denStudii", name: formatMessage({ id: "globals.denStudii" }) }
    ]
};
 ...
 */
const getRowValues = (entries: StudiiDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const StudiiTable = () => {
    ...
    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <StudiiAddDialog /> {/* Add the button to open the Studii add modal. */}
                ...
                <TableBody>
                    {
                        rowValues?.map(({ data, entry }, rowIndex) => <TableRow key={`row_${rowIndex + 1}`}>
                            ...
                            <TableCell> {/* Add other cells like action buttons. */}
                                <StudiiUpdateDialog pid = {String(entry.id)}/>
                        </TableCell>
 {/*                       <TableCell>  Add other cells like action buttons. 
...
*/}
                       <TableCell>  {/* Add other cells like action buttons. */}
                            <ConfirmProvider>
                                <ConfirmDialogStudii pid = {entry.id}/>
                            </ConfirmProvider>
                        </TableCell>                        
